...
# ── int6 quantization with true 6-bit packing ────────────────────────────────
INT6_CLIP_Q = 0.9999
...
def _pack6(q_int8):
    """Pack int8 values in [-32,31] into 6 bits. 4 values -> 3 bytes."""
    flat=(q_int8.reshape(-1).to(torch.int16)+32).to(torch.uint8)  # shift to [0,63]
    pad=(-len(flat))%4; flat=torch.cat([flat,torch.zeros(pad,dtype=torch.uint8)]) if pad else flat
    f=flat.reshape(-1,4)
    b0=((f[:,0]<<2)|(f[:,1]>>4)).to(torch.uint8)
    b1=(((f[:,1]&0xF)<<4)|(f[:,2]>>2)).to(torch.uint8)
    b2=(((f[:,2]&0x3)<<6)|f[:,3]).to(torch.uint8)
    return torch.stack([b0,b1,b2],dim=1).reshape(-1).numpy().tobytes(),len(q_int8.reshape(-1))

def _unpack6(data,n):
    """Unpack 6-bit packed bytes -> int8 values in [-32,31]."""
    raw=np.frombuffer(data,dtype=np.uint8).copy()
    raw=torch.from_numpy(raw).reshape(-1,3)
    v0=(raw[:,0]>>2).to(torch.int8)
    v1=(((raw[:,0]&0x3)<<4)|(raw[:,1]>>4)).to(torch.int8)
    v2=(((raw[:,1]&0xF)<<2)|(raw[:,2]>>6)).to(torch.int8)
    v3=(raw[:,2]&0x3F).to(torch.int8)
    ...
    for k,v in sd.items():
        ...
        if is_ctrl:
            ...
            meta[k]={"k":"q6","s":list(v.shape),"ss":list(s.shape),"nv":nvals,
                     "qo":buf.tell(),"qn":len(packed),"so":buf.tell()+len(packed),"sn":len(sb)}
            buf.write(packed); buf.write(sb)
...
def dequantize_state_dict(meta,raw):
    ...
    for k,m in meta.items():
        if m["k"]=="f":
            ...
            q=_unpack6(raw[m["qo"]:m["qo"]+m["qn"]],m["nv"])
            s=torch.from_numpy(np.frombuffer(raw[m["so"]:m["so"]+m["sn"]],dtype=np.float16).copy())
            q=q.view(m["ss"][0] if m["ss"] else 1,-1); s=s.view(m["ss"])
...
def main():
    ...
    while True:
        ...
        if a.ema_enabled:
            with torch.no_grad():
                bm_sd=bm.state_dict()
                for k,v_ema in ema.state_dict().items():
                    bk=k if k in bm_sd else "_orig_mod."+k
                    if bk in bm_sd: v_ema.mul_(a.ema_decay).add_(bm_sd[bk],alpha=1.0-a.ema_decay)
        # Tight SWA
        ...
        # No late QAT — our fake_quant destroys training. Int6 applied post-training only.
